  ...
  if (!user) {
    return (
      <p className="text-destructive">
        You&apos;re not auhtorized to view this page.
      </p>
    );
  }
...
async function UserInfoSidebar({ user }: UserInfoSidebarProps) {
  ...
  return <div className="space-y-5 rounded-2xl bg-card p-5 shadow-sm">
    ...
    <UserTooltip user={user}>
      <Link href={`/users/${user.username}`} className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="line-clamp-1 break-all text-muted-foreground font-semibold hover:underline">@{user.username}</p>
        </div>
      </Link>
    </UserTooltip>
    <Linkify>
    ...
    {user.id !== loggedInUser.id && (
      <FollowButton userId={user.id} intialState={{followers: user._count.followers, isFollowedByUser: user.followers.some(
